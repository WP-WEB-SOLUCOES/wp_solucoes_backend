...
// CORRIGIDO: Adicionado 'admin_join'
...
        window.location.href = '/admin/login'; // Redireciona para a URL limpa
        return;
    }
    
    // 2. Elementos da "Casca" (main.html)
...
    }
    
    // Botão de Logout agora limpa o cookie no servidor
    if (logoutButton) {
...
    
    socket.on('connect', () => {
        console.log('Conectado ao servidor, anunciando como admin...');
        // === CORREÇÃO AQUI ===
        socket.emit('admin_join'); // Anuncia que este é um socket de atendente
        // =====================
    });

    // --- Eventos de Chat de Cliente ---
    socket.on('update_client_list', (clientListArray) => {
        // Mapeia a lista recebida (que é o array de 'clients.values()' do servidor)
        clients = clientListArray.reduce((acc, clientData) => {
            if (clientData.session) {
                acc[clientData.session.sessionId] = clientData;
            }
            return acc;
        }, {});
        
...
        
        // Garante que o cliente existe no mapa local
        if (!clients[fromSession]) {
            // Se não existir, é um cliente novo, mas precisamos dos dados da sessão.
            // O 'update_client_list' que virá em seguida irá criá-lo.
            // Por agora, vamos apenas preparar a estrutura.
            clients[fromSession] = { session: { sessionId: fromSession, clientData: { name: fromName }, messages: [] } };
        }
        
...
            }
            
            // Atualiza a UI da lista de clientes
            const clientItem = document.querySelector(`.client-item[data-id="${fromSession}"]`);
            if (clientItem) {
                // Apenas atualiza a última mensagem se não estiver atribuído
...
                }
                // Mostra o badge de notificação se não for o chat selecionado
                if (fromSession !== selectedSessionId) {
...
            // Se for a própria mensagem, apenas exibe se estiver na página
...
        }
        
        // Se for de outro atendente, exibe se estiver na página
        const messagesUI = document.getElementById('internalMessages');
        if (messagesUI) {
            appendInternalMessage(data);
        } else {
            // Ou mostra notificação se não estiver na página
...
    
    // Ordena pelo 'updatedAt' da sessão
...
        
        // O servidor envia 'client.socketId' (que é o 'socketId' do objeto 'client' no servidor)
...
        
        // Verifica o 'socketId' do objeto 'client'
        if (client.socketId) {
            // Cliente está online
...
            
            } else {
                // Online, mas atribuído a outro atendente (e eu não sou admin)
...
        messageEl.className = 'message client message-internal';
        const avatarDiv = document.createElement('div');
        avatarDiv.className = 'avatar';
...
    } 
    else {
        messageEl.className = 'message atendente message-internal';
        const avatarDiv = document.createElement('div');
        avatarDiv.className = 'avatar';
...
                        window.location.href = '/admin/login';
                    }
                  mregisterMessage.textContent = data.message || 'Erro ao registrar.';
